...
const DetallesTransaccion = ({
  ...
  // Obtener estados para el desplegable
  const { data: estadosDetalle, isLoading: loadingEstados } =
    useGetEstadosDetalleQuery();
  useEffect(() => {
    ...
    const detallesInicializados =
      detallesIniciales?.map((detalle) => ({
        ...
        estado_producto_transaccion: detalle.estado_producto_transaccion || 0,
      ...
      {detalles.map((detalle, index) => (
        <Box key={index} display="flex" alignItems="center" gap={2} mb={2}>
          ...
          <TextField
            ...
            style={{ flex: 1 }}
          />
          {/* Desplegable para Estado Detalle */}
          <Select
            value={
              estadosDetalle?.find(
                (estado) =>
                  estado.id_estado_detalle_transaccion ===
                  detalle.estado_producto_transaccion
              )?.id_estado_detalle_transaccion || "" // Proporciona "" si no coincide
            }
            onChange={(e) =>
              handleDetalleChange(
                index,
          ...
          >
            ...
            {!loadingEstados &&
              estadosDetalle?.map((estado) => (
                <MenuItem
                  key={estado.id_estado_detalle_transaccion}
                  value={Number(estado.id_estado_detalle_transaccion)}
                >
